...
const appColorDict = (0, tailwind_helper_1.generateColorDict)(appColors);
...
function showReport(context, node, tagValue) {
    context.report({
        ...
        fix: function (fixer) {
            return fixer.replaceTextRange([node.range[0] + 1, node.range[1] - 1], getOriginalTWClassNames(tagValue, appColorDict));
        },
        suggest: [
            {
