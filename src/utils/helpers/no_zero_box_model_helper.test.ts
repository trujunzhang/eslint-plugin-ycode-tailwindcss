...
describe("function in the NoZeroBoxModelHelper", () => {
  it("isBoxModelZero", () => {
    ...
    // result:[false]
    // expect(new NoZeroBoxModelHelper().isBoxModelZero("mt-[0pxxx] mb-[0px] ml-[0] mr-[0px] flex sm:pr-[0px] sm:gap-[12px]")).toBe(false);
    // expect(new NoZeroBoxModelHelper().isBoxModelZero("sm:pr-[0px] sm:gap-[12px]")).toBe(false);
    // result:[true]
    // expect(new NoZeroBoxModelHelper().isBoxModelZero("mt-[0] mb-[0] ml-[0] mr-[0] flex sm:pr-[0px] sm:gap-[12px]")).toBe(true);
    expect(
      new NoZeroBoxModelHelper().isBoxModelZero(
        "max-w-[80rem] pt-[0] pb-[0px] pl-[0] pr-[0px] flex sm:pr-[0px] sm:gap-[12px]",
      ),
    ).toBe(true);
  });
});
